...
(function () {
  ...
  function makePart(born) {
    const c = COLORS[Math.floor(Math.random() * COLORS.length)];
    return {
      x: Math.random() * W,
      y: born ? Math.random() * H : H + 5,
      r: .5 + Math.random() * 2.2,
      vx: (Math.random() - .5) * .22,
  ...
  function skyAt(t) {
    const keys = [
      ...
      { t: .2,   top:[30,22,18],  bot:[165,100,50] },
      { t: .42,  top:[38,28,20],  bot:[100,68,32]  },
      { t: .62,  top:[28,18,12],  bot:[175,108,44] },
      { t: .78,  top:[14,9,7],    bot:[145,70,28]  },
      { t: .9,   top:[10,7,5],    bot:[90,45,18]   },
    ...
    const mix = (ca,cb) => [0,1,2].map(i => Math.round(ca[i]+(cb[i]-ca[i])*f));
  ...
  function drawGlow() {
    const cx = W*.5 + (mouse.x-.5)*W*.12;
    const cy = H*.52 + (mouse.y-.5)*H*.07;
    const p = .16 + .07*Math.sin(clock*.0028);
    const g = ctx.createRadialGradient(cx,cy,0,cx,cy,Math.max(W,H)*.82);
    g.addColorStop(0,  `rgba(200,130,40,${p})`);
    g.addColorStop(.4, `rgba(200,130,40,${p*.28})`);
  ...
  function drawFog() {
    for (let i = 0; i < 3; i++) {
      const y = H*(.44+i*.13) + Math.sin(clock*.0007+i*1.3)*18;
      const a = .035 + .015*Math.sin(clock*.001+i);
      const g = ctx.createLinearGradient(0,y-55,0,y+55);
      g.addColorStop(0,'rgba(210,196,185,0)');
      g.addColorStop(.5,`rgba(210,196,185,${a})`);
  ...
  const cracks = [];
  function makeCrack() {
    let x=Math.random()*W, y=H*.55+Math.random()*H*.45;
    const pts=[{x,y}];
    for(let i=0;i<4+Math.floor(Math.random()*6);i++){
      x+=(Math.random()-.5)*80; y+=10+Math.random()*38;
      pts.push({x,y});
    }
    return{pts,prog:0,spd:.003+Math.random()*.005,op:0,target:.04+Math.random()*.07};
  }
  function drawCracks() {
    cracks.forEach(c=>{
      c.prog = Math.min(1,c.prog+c.spd);
      c.op += (c.target-c.op)*.018;
      ctx.beginPath();
      ctx.strokeStyle=`rgba(155,115,75,${c.op})`;
      ctx.lineWidth=.5;
      ctx.setLineDash([2,4]);
      const{pts}=c,n=(pts.length-1)*c.prog,full=Math.floor(n),frac=n-full;
  ...
  function drawParts() {
    parts.forEach((p,i)=>{
      p.life++;
      p.wob+=p.wSpd;
      p.x+=p.vx+Math.sin(p.wob)*.28;
      p.y+=p.vy;
      const lf=p.life/p.max;
      const a=lf<.1?p.a*(lf/.1):lf>.8?p.a*(1-(lf-.8)/.2):p.a;
      ctx.beginPath();
      ctx.arc(p.x,p.y,p.r,0,Math.PI*2);
      ctx.fillStyle=`rgba(${p.col},${a})`;
      ctx.fill();
      if(p.life>=p.max||p.y<-10) parts[i]=makePart(false);
    });
  }

  function drawVignette() {
    const g=ctx.createRadialGradient(W/2,H/2,H*.08,W/2,H/2,H*.85);
  ...
  function frame() {
    clock++;
    if(clock%120===0) buildGrain();
    const t=(clock*.00022)%1;
    drawBg(t);
    drawGlow();
    drawFog();
    drawCracks();
  ...
  function resize() {
    ...
    if(grain){grain.width=W;grain.height=H;}
  }
  ...
  function init() {
    ...
    parts.length=0;
    for(let i=0;i<NUM;i++) parts.push(makePart(true));
    buildGrain();
    frame();
  }
